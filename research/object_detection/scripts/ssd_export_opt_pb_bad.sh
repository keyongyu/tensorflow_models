...
SEQ_NO=$2
OPT_PB_FILE=$3
QUANT_OPT_PB_FILE=$3.quantized.pb
...
transform_graph --in_graph=/tmp/my.pb/frozen_inference_graph.pb  --out_graph="$OPT_PB_FILE" \
--inputs='image_tensor' \
--outputs='detection_boxes,detection_scores,detection_classes,num_detections' \
--transforms='
  fold_constants(ignore_errors=true) fold_batch_norms fold_old_batch_norms
  round_weights(num_steps=256) remove_nodes(op=Identity)
  strip_unused_nodes sort_by_execution_order'

transform_graph --in_graph=/tmp/my.pb/frozen_inference_graph.pb  --out_graph="$QUANT_OPT_PB_FILE" \
--inputs='image_tensor' \
--outputs='detection_boxes,detection_scores,detection_classes,num_detections' \
--transforms=' 
